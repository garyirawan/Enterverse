...
export default function MoviesScreen({ navigation }) {
  ...
  const fetchMovies = async () => {
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/popular?api_key=9634e103c05aa8d8623d8a6385ecd62c&language=en-US&page=${page}`
      );
  ...
  const renderCard = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('DetailScreen', { item, type: 'movie' })}
    >
